...
export default function ZamanlanamaPage() {
  ...
  async function applySchedule() {
    ...
    setShowPreview(false);
    // Refresh list
    fetch("/api/vippanel/zamanlama").then((r) => r.json()).then((data: PostItem[]) => setPosts(data));
  ...
  return (
    <div className="p-6 md:p-8 pt-16 md:pt-8">
      ...
      <div className="mt-6 p-4 bg-zinc-100 rounded-xl text-xs text-zinc-500 space-y-1">
        ...
        <p>Tarihleri değiştirmek yazı içeriğini etkilemez. İstediğiniz zaman <Link href="/vippanel/blog" className="underline">Blog sayfasından</Link> manuel düzenleyebilirsiniz.</p>
      </div>
    </div>
  );
}
