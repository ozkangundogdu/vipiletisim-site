...
export default function MarkaTamirDetayPage() {
  ...
  useEffect(() => {
    fetch(`/api/vippanel/marka-tamirler/${slug}`)
      .then((r) => r.json())
      .then((d) => {
        setData(d);
  ...
  async function kaydet(guncelModeller: DesteklenenModel[]) {
    setSaving(true);
    await fetch(`/api/vippanel/marka-tamirler/${slug}`, {
      method: "POST",
  ...
  return (
    <div className="p-6 md:p-8 pt-16 md:pt-8 max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/vippanel/marka-tamirler" className="text-zinc-400 hover:text-zinc-700 transition-colors">
          ← Geri
        </Link>
        <div>
