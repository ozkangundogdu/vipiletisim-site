...
import { getCustomServices } from "@/lib/custom-services";
...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: "Blog Yazısı",       value: posts.length,           href: "/vippanel/blog",         color: "#3b82f6", icon: "📝" },
    { label: "Müşteri Yorumu",    value: reviews.length,         href: "/vippanel/yorumlar",     color: "#f59e0b", icon: "⭐" },
    { label: "SSS Sorusu",        value: faq.length,             href: "/vippanel/sss",          color: "#10b981", icon: "❓" },
    { label: "Marka Tamir Sayfası", value: totalRepairPages,      href: "/vippanel/marka-tamirler", color: "#ff351b", icon: "📱" },
  ];
  ...
  return (
    <div className="p-6 md:p-8 pt-16 md:pt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-zinc-100">
          ...
          <div className="space-y-2">
            {[
              { href: "/vippanel/blog/yeni",         label: "Yeni Blog Yazısı Ekle",        icon: "✏️" },
              { href: "/vippanel/marka-tamirler",    label: "Marka Tamir Sayfaları",        icon: "📱" },
              { href: "/vippanel/iletisim",          label: "İletişim Bilgilerini Düzenle", icon: "⚙️" },
              { href: "/vippanel/yorumlar",          label: "Yorum Yönetimi",               icon: "⭐" },
              { href: "/vippanel/sss",               label: "SSS Yönetimi",                 icon: "❓" },
            ].map((item) => (
              <Link
                key={item.href}
                href={item.href}
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-zinc-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/vippanel/blog" className="text-xs font-bold" style={{ color: "#ff351b" }}>Tümü →</Link>
          </div>
          <ul className="space-y-2">
            {recentPosts.length === 0 ? (
              ...
              <li key={post.slug}>
                <Link href={`/vippanel/blog/${post.slug}`} className="flex items-start gap-2 p-2 rounded-lg hover:bg-zinc-50 transition-colors">
      ...
      {/* Marka Tamir Sayfaları Özeti */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-zinc-100">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-black text-zinc-900">Marka Tamir Sayfaları</h2>
            <Link href="/vippanel/marka-tamirler" className="text-xs font-bold" style={{ color: "#1A3A6B" }}>Yönet →</Link>
          </div>
          <div className="space-y-3">
            {["iphone", "samsung", "xiaomi"].map((marka) => {
              const count = markaTamirler.filter((m) => m.marka === marka).length;
              const label = marka === "iphone" ? "iPhone" : marka === "samsung" ? "Samsung" : "Xiaomi";
              const renk = marka === "iphone" ? "#111" : marka === "samsung" ? "#1428A0" : "#FF6900";
              return (
                <div key={marka} className="flex items-center justify-between py-2.5 border-b border-zinc-50 last:border-0">
                  <div className="flex items-center gap-2.5">
                    <span className="w-2 h-2 rounded-full shrink-0" style={{ background: renk }} />
                    <span className="text-sm font-bold text-zinc-700">{label} Tamir Sayfası</span>
                  </div>
                  <span className="text-sm font-black text-zinc-900">{count}</span>
                </div>
              );
            })}
          </div>
          <Link
            href="/vippanel/marka-tamirler"
          ...
          >
            Marka Tamir Sayfalarını Yönet
          </Link>
        </div>

        {/* Son Blog Yazıları (sağ kolon) */}
        <div className="bg-white rounded-xl p-6 shadow-sm border border-zinc-100">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base font-black text-zinc-900">Tamir Sayfası İstatistikleri</h2>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2.5 border-b border-zinc-50">
              <span className="text-sm font-bold text-zinc-700">Toplam Marka Tamir Sayfası</span>
              <span className="text-sm font-black text-zinc-900">{markaTamirler.length}</span>
            </div>
            <div className="flex items-center justify-between py-2.5 border-b border-zinc-50">
