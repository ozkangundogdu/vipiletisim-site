...
export default function VideolarPage() {
  ...
  useEffect(() => {
    fetch("/api/vippanel/videos").then(r => r.json()).then(data => { setVideos(data); setLoading(false); });
  }, []);
  ...
  async function fetchByUrl() {
    ...
    try {
      const res = await fetch("/api/vippanel/youtube-fetch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "url", url: botUrl.trim() }) });
  ...
  async function searchVideos() {
    ...
    try {
      const res = await fetch("/api/vippanel/youtube-fetch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "search", query: botQuery.trim() }) });
  ...
  async function fetchChannel() {
    ...
    try {
      const res = await fetch("/api/vippanel/youtube-fetch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "channel", handle: channelHandle.trim(), filter: channelFilter }) });
      const data = await res.json() as { error?: string; results?: YTResult[] };
      if (data.error) { setBotError(data.error); return; }
      ...
      for (const r of fresh) {
        fetch("/api/vippanel/youtube-fetch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "url", url: r.url }) })
          .then(res => res.json())
  ...
  async function commitStaging(publishNow = false) {
    ...
    setSaving(true);
    await fetch("/api/vippanel/videos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(allVideos) });
    setSaving(false); setSaved(true);
    setTimeout(() => setSaved(false), 3000);

    // Thumbnail'ları arka planda indir
    for (const v of newVideos) {
      fetch("/api/vippanel/videos/thumbnail", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: v.id, platform: "youtube", videoId: v.videoId }) })
  ...
  async function fetchStagingDesc(sv: StagingVideo) {
    ...
    try {
      const res = await fetch("/api/vippanel/youtube-fetch", {
  ...
  async function addSelectedVideo() {
    ...
    fetch("/api/vippanel/videos/thumbnail", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, platform: "youtube", videoId: selectedVideo.videoId }) })
  ...
  async function addVideo() {
    ...
    setAddForm(null); resetBot();
    fetch("/api/vippanel/videos/thumbnail", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, platform: addForm.platform, videoId }) })
      .then(r => r.json()).then((d: { path: string | null }) => { if (d.path) setVideos(prev => prev.map(v => v.id === id ? { ...v, thumbnail: d.path ?? undefined } : v)); }).catch(() => {});
  }
  ...
  async function handleSave() {
    ...
    await fetch("/api/vippanel/videos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(videos) });
    setSaving(false); setSaved(true);
    setTimeout(() => setSaved(false), 2500);
  }
    ...
    <div className="p-6 md:p-8 pt-16 md:pt-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <a href="/vippanel/videolar/zamanlama" className="px-4 py-2 rounded-lg text-sm font-black border border-zinc-200 text-zinc-600 hover:border-zinc-400 transition-colors">📅 Zamanla</a>
                                ...
                                <div className="flex items-center gap-3 p-2">
                                  ...
                                  <button
                                    onClick={async e => {
                                      ...
                                      fetch("/api/vippanel/videos/thumbnail", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, platform: "youtube", videoId: r.videoId }) })
