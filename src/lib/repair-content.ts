...
const generators: Record<string, Generator> = {
  ...
  'ekran-degisimi': (model) => ({
    ...
    why: `Kırık ekranla kullanıma devam edildiğinde cam kesikleri yaralanma riskine yol açar; aynı zamanda LCD veya OLED panelin zarar görme ihtimali her geçen gün artar. Çatlak camdan giren toz ve nem, anakartı olumsuz etkileyebilir. Erkenden yapılan ${model} ekran değişimi, hem daha ekonomik hem de cihazınızın uzun vadeli sağlığı açısından kritik öneme sahiptir.`,
    ...
    processSteps: [
      ...
      { title: 'Orijinal Panel Takımı', desc: 'OEM veya orijinal kalitesinde ekran modülü, özel ısı ve vakum aparatlarıyla hassasiyetle yerleştirilir.' },
    ...
    expertNote: 'OLED ekranlı modellerde muadil panel kullanımı renk doğruluğunu ve True Tone işlevini olumsuz etkileyebilir. Bu nedenle Vip İletişim, OLED cihazlar için OEM kalite panel tercih etmektedir.',
  }),

  'batarya-degisimi': (model) => ({
    intro: `${model} batarya değişimi, cihazın performansını ve günlük kullanım süresini doğrudan etkileyen kritik bir tamir işlemidir. Lityum-iyon piller her şarj döngüsünde küçük bir kapasite kaybeder; belirli bir eşiğin altına düştüğünde cihazı yavaşlatabilir ya da beklenmedik kapanmalara yol açabilir. Trabzon'da ${model} batarya değişimi için Vip İletişim Teknik Servis'i tercih eden kullanıcılar cihazlarını aynı gün teslim almaktadır.\n\nOrijinal kapasite değerlerine sahip yeni batarya takıldığında telefon sanki yeni çıkmış gibi çalışmaya başlar. Şarj süreleri ve performans kısıtlamaları ortadan kalkar.`,
    ...
    symptoms: [
      ...
      'iPhone pil sağlığı %80\'in altına düşmüş',
      'Cihaz yoğun kullanımda aşırı ısınıyor',
    ],
    whyHeading: `${model} Batarya Neden Zamanında Değiştirilmeli?`,
    why: `Şişen bir batarya yalnızca performans kaybına değil, ekrana ve anakarta fiziksel baskı yaparak çok daha pahalı hasarlara da yol açabilir. İleri düzeyde şişme, yangın riski taşıyan tehlikeli bir durumdur. ${model} batarya değişimini zamanında yaptırmak, yüzlerce liralık ekstra tamiri önler ve cihazınızın güvenli kullanımını sürdürür.`,
    ...
    processSteps: [
      ...
      { title: 'Orijinal Kapasite Batarya Takımı', desc: `Sertifikalı, orijinal kapasite değerlerine sahip batarya takılır.` },
    ...
    expertNote: 'Şişen bataryayla cihazı şarj etmeyin; ısı kaynağına yaklaştırmayın. Servisimize en kısa sürede getirmenizi öneririz.',
  }),

  'sarj-soketi-tamiri': (model) => ({
    intro: `${model} şarj soketi tamiri, cihazın en temel işlevini — şarj olmayı ve veri aktarımını — doğrudan etkileyen önemli bir tamir hizmetidir. Şarj soketleri günlük takılıp çıkarma işlemleriyle yıpranır; ayrıca toz, kir ve nem birikimi bağlantı sorunlarına yol açar. Trabzon'da ${model} şarj soketi tamiri için Vip İletişim'e başvurabilirsiniz.\n\nÜcretsiz ön inceleme sırasında soketin fiziksel durumu ve anakart bağlantısı test edilir; temizlik mi yoksa değişim mi gerektiği netleştirilir.`,
    symptomsHeading: `${model} Şarj Soketi Arızasının Belirtileri`,
    symptoms: [
      'Kablo takıldığında şarj simgesi görünmüyor veya geç görünüyor',
  ...
  'on-kamera-tamiri': (model) => ({
    intro: `${model} ön kamera tamiri; selfie fotoğrafçılığı, video görüşme ve bazı modellerde Face ID işlevini doğrudan etkiler. Ön kamera arızaları düşme, darbe veya nem sonrasında ortaya çıkabileceği gibi üretim kaynaklı erken yıpranma sonucunda da oluşabilir. Trabzon'da ${model} ön kamera tamiri için Vip İletişim orijinal kamera modülü kullanarak görüntü kalitesini ve biyometrik güvenlik özelliklerini tam olarak yeniler.\n\nÖn kamera ile Face ID sistemi çoğunlukla aynı modülde yer aldığından, değişimin doğru yapılması kritik önem taşır.`,
    ...
    symptoms: [
      ...
      'Face ID "Ayarlanamadı" hatası veriyor (bazı modellerde ön kamera bağlantılı)',
      'Kamera uygulaması ön kameraya geçildiğinde çöküyor',
    ],
    whyHeading: `${model} Ön Kamera Değişimi Neden Önemli?`,
    why: `Ön kamera modülü yalnızca fotoğraf değil; video konferans, biyometrik kimlik doğrulama ve belge tarama işlevlerini de yürütür. Düşük kaliteli veya uyumsuz modül takılması görüntü kalitesini kalıcı olarak düşürebilir ve Face ID'nin çalışmamasına neden olabilir. ${model} için orijinal ya da OEM kalitesinde ön kamera kullanılması bu riskleri ortadan kaldırır.`,
    ...
    processSteps: [
      ...
      { title: 'Hassas Söküm', desc: 'Ekran ve sensör şeritlerine zarar vermeden kamera modülü ayrılır.' },
      { title: 'Orijinal Modül Takımı', desc: 'OEM kalitesinde ön kamera yerleştirilir.' },
      { title: 'Biyometrik ve Görüntü Kontrolü', desc: 'Face ID / parmak izi çalışması ve görüntü netliği doğrulanır.' },
    ],
    dataSafeHeading: `Ön Kamera Değişiminde Verilerim Silinir mi?`,
    dataSafe: `${model} ön kamera değişimi donanımsal bir işlemdir. Verileriniz ve ayarlarınız tamamen korunur. Face ID yeniden kayıt gerektirebilir.`,
    ...
    expertNote: 'Ön kamera değişimi Face ID bileşenlerini etkiler; deneyimsiz müdahale Face ID\'yi kalıcı olarak devre dışı bırakabilir.',
  }),

  'arka-kamera-tamiri': (model) => ({
    intro: `${model} arka kamera tamiri; ana kamera, ultra geniş açı ve telefoto lensleri kapsayan kapsamlı bir teknik hizmettir. Modern akıllı telefonların çok lensli kamera sistemleri, darbe ve nem gibi dış etkenlerden daha kolay etkilenebilir. Trabzon'da ${model} arka kamera tamiri için Vip İletişim orijinal kamera modülleri kullanarak görüntü kalitesini tamamen yeniler.\n\nSorunun lens mi, sensör mü yoksa anakart bağlantısı mı kaynaklandığı ücretsiz tanı aşamasında netleştirilir; yalnızca arızalı bileşen değiştirilir.`,
    ...
    processSteps: [
      { title: 'Çok Lensli Sistem Testi', desc: 'Geniş, ultra geniş ve zoom kameralar ayrı ayrı test edilir.' },
    ...
    price: `Arka kamera tamiri ücreti; hangi lensin (ana, geniş açı, zoom) arızalı olduğuna göre değişir. Ücretsiz ön inceleme sonrası net fiyat bildirilir.`,
    expertNote: 'Çok kameralı sistemlerde yalnızca arızalı modül değiştirilir; çalışan modüllere gereksiz müdahaleden kaçınılır.',
  }),

  'face-id-tamiri': (model) => ({
    intro: `${model} Face ID tamiri, Apple'ın yalnızca iPhone'lara özel TrueDepth kamera sistemiyle ilgili bir hizmettir. Face ID; kızılötesi kamera, flood illuminator, nokta projektör ve yakınlık sensöründen oluşan karmaşık bir biyometrik sistemdir. Bu sistemdeki herhangi bir bileşenin arızalanması yüz tanımanın tamamen durmasına neden olur. Trabzon'da ${model} Face ID tamiri için Vip İletişim, Apple prosedürlerine uygun teşhis ve onarım uygular.\n\nFace ID arızası bazen yazılımdan, bazen ekran değişiminden, bazen de TrueDepth modülünün fiziksel hasarından kaynaklanır. Doğru tanı olmadan yapılan müdahale bu özelliği kalıcı olarak devre dışı bırakabilir.`,
    ...
    symptoms: [
      ...
      'Düşme sonrası ön sensör grubu hasar gördü',
    ...
    expertNote: 'Face ID tamiri yalnızca iPhone X ve sonrası modelleri kapsar. Samsung ve Xiaomi cihazlarda Face ID teknolojisi bulunmamaktadır.',
  }),

  'kamera-cami-tamiri': (model) => ({
    ...
    symptoms: [
      ...
      'Yalnızca belirli kamera (ana, ultra geniş, zoom) etkilenmiş görünüyor',
  ...
  'kasa-degisimi': (model) => ({
    intro: `${model} kasa değişimi; düşme, çarpma veya uzun süreli kullanımdan kaynaklanan yapısal hasarı tamamen gideren kapsamlı bir teknik hizmettir. Ezik, bükülmüş ya da kırılmış kasa yalnızca görsel bir sorun gibi görünse de iç bileşenlere kronik baskı uygulayarak zamanla anakart, batarya ve ekran arızalarına yol açabilir. Trabzon'da ${model} kasa değişimi için Vip İletişim orijinal kalite kasa parçaları kullanır.\n\nKasa değişimi diğer tamirlerden daha uzun sürer; ancak cihazı neredeyse yeni görünüme kavuşturur.`,
    ...
    symptoms: [
      ...
      'Su geçirmezlik contası hasarlanmış, koruma özelliği ortadan kalkmış',
    ...
    processSteps: [
      ...
      { title: 'Yeni Kasa Montajı', desc: 'Orijinal kalite kasa montajlanır; tüm bileşenler yeni kasaya yerleştirilir.' },
    ...
    expertNote: 'Kasa değişimi yaklaşık 2–4 saat sürer; aynı gün randevu alınması durumunda gün içinde teslim yapılabilir.',
  }),

  'arka-kapak-tamiri': (model) => ({
    intro: `${model} arka kapak tamiri, cihazın su geçirmezliğini ve darbe korumasını yenileyen önemli bir hizmettir. Cam arka kapak kırığı; toz, nem ve darbe geçişine kapı açarak iç bileşenleri riske atar. Trabzon'da ${model} arka kapak tamiri için Vip İletişim, orijinal kalite arka cam veya kapak kullanarak cihazı fabrika görünümüne döndürür.\n\nArka kapak değişimi görece hızlı bir işlemdir ve çoğunlukla aynı gün tamamlanır.`,
    ...
    symptoms: [
      ...
      'IP su ve toz geçirmezlik özelliği ortadan kalkmış',
    ...
    why: `Arka kapak, cihazın dış kabuğundaki en ince ve en kırılgan bölümdür. Kırık cam parçaları elleri zedelerken aynı zamanda içeri sızan nem anakartı ve bataryayı tehdit eder. IP derecelendirmeli bir modelde arka kapak kırığı güvenlik sertifikasını geçersiz kılar. ${model} arka kapak tamiri bu riskleri ortadan kaldırır ve cihazın güvenli kullanımını sağlar.`,
  ...
  'sivi-temasi-tamiri': (model) => ({
    intro: `${model} sıvı teması tamiri, her dakikanın kritik öneme sahip olduğu acil bir müdahale hizmetidir. Suya ya da başka bir sıvıya temas eden cihazda oksidasyon (korozyon) birkaç dakika içinde başlar; anakart, ekran bağlantıları ve sensörler sırayla etkilenmeye başlar. Trabzon'da ${model} sıvı teması tamiri için Vip İletişim ultrasonik temizleme ve korozyon giderme teknolojisiyle müdahale eder.\n\nCihazınız suya düştüyse hemen kapatın, şarj etmeyin ve en kısa sürede servisimize getirin. Erken müdahale başarı şansını önemli ölçüde artırır.`,
  ...
  'sarj-olmuyor-tamiri': (model) => ({
    ...
    symptoms: [
      ...
      'Şarj soketine bir cisim girilmiş ve pin hasar görmüş',
    ],
    ...
    processSteps: [
      { title: 'Kablo ve Adaptör Doğrulaması', desc: 'Farklı kaliteli kablo ve adaptörle test yapılır; aksesuar kaynaklı sorun elenir.' },
      { title: 'Soket Temizliği ve Pin Kontrolü', desc: 'Şarj soketindeki toz temizlenir, pinler kontrol edilir.' },
  ...
  'sarj-entegresi-tamiri': (model) => ({
    ...
    why: `Şarj entegresi, anakart yüzeyindeki 1–2 mm boyutunda bir BGA çiptiir. Değişimi; trinoküler mikroskop, ısı tabancası ve reballing ekipmanı gerektiren mikro lehimleme uzmanlığı ister. Deneyimsiz müdahale entegrenin yanı sıra çevresindeki bileşenleri de hasar görmesine yol açar. Vip İletişim ${model} şarj entegresi tamirini bu konuda uzmanlaşmış kadrosuyla Trabzon'da başarıyla uygulamaktadır.`,
  ...
  'on-cam-degisimi': (model) => ({
    intro: `${model} ön cam değişimi; ekranın alt katmanı (LCD veya OLED) sağlamken yalnızca üstteki dokunmatik cam tabakasının yenilenmesini kapsayan ekonomik bir tamir hizmetidir. Ön cam kırıldığında görüntü normal olabilir; ancak cam parçaları dokunmatik hassasiyetini bozar, elleri çizer ve nem sızmasına davetiye çıkarır. Trabzon'da ${model} ön cam değişimi için Vip İletişim laminasyon teknolojisiyle hem ekonomik hem de dayanıklı bir çözüm sunar.\n\nTam ekran değişimi yerine yalnızca cam değişimi yapılabilmesi önemli maliyet avantajı sağlar.`,
    symptomsHeading: `${model} Ön Cam Değişimi Gerektiğinin Belirtileri`,
    symptoms: [
      'Ekran camı çatlamış ancak LCD/OLED görüntüsü normal ve net',
    ...
    why: `Tam ekran değişimi LCD/OLED + cam + çerçeve bütünü değiştirildiğinde uygulanır. Ön cam değişimi ise yalnızca dış cam tabakasının laminasyon yöntemiyle yenilenmesidir; LCD/OLED dokunulmaz. ${model} için doğru tanı; gereksiz tam ekran değişimini önleyerek önemli tasarruf sağlar. Erken müdahale önemlidir; çünkü devam eden kullanım LCD'nin de hasar görmesine yol açar.`,
    processHeading: `${model} Ön Cam Değişimi Süreci`,
    processSteps: [
      { title: 'LCD/OLED Durum Testi', desc: 'Alt panel hasarsız mı kontrol edilir; laminasyon için uygunluk değerlendirilir.' },
    ...
    expertNote: 'LCD veya OLED alt panel hasarlıysa ön cam değişimi yeterli olmaz; tam ekran değişimi gerekir. Doğru tanı bu ayrımı netleştirir.',
  }),
};

const fallback: Generator = (model, _brand) => ({
...
export function getRepairContent(model: string, brand: string, repairKey: string): RepairContent {
  const gen = generators[repairKey] ?? fallback;
  return gen(model, brand);
}
